    ...
    <div class="container py-8">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach ($posts as $post)
                {{-- para obtener la url completa de las imagenes usamos storage--}}
                <article class="w-full h-80 bg-cover bg-center @if($loop->first) md:col-span-2 @endif"
                        style="background-image: url(@if($post->image) {{ Storage::url($post->image->url) }}
                                                     @else https://cdn.pixabay.com/photo/2022/11/14/20/14/compass-7592447_960_720.jpg @endif)">
                        <div class="w-full h-full px-8 flex flex-col justify-center">
                            ...
                            <h1 class="text-4xl text-white leading-8 font-bold mt-2">
                                <a href="{{ route('post.show', $post) }}">{{ $post->name }}</a></h1>
                        </div>
                </article>
            @endforeach
        </div>
